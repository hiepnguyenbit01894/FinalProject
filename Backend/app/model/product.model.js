...
product.getAll = function (page, page_size, sort, params_sort, from, to, result) {
    let total;
        ...
        if (page || page_size) {
            db.query("SELECT * FROM products", function (err, product) {
                if (err) {
                    ...
                    if (sort || params_sort) {
                        if (to != 0 && from != 0 && sort == 1) {
                            db.query(`SELECT * FROM products Where price between ${from} and ${to} LIMIT ? OFFSET ? `, [parseInt(page_size), offset], function (err, product) {
                                console.log("asdfadsf",sort2);

                                if (err) {
                                    console.log(err);
                                } else {
                                    product.totalPage = Math.ceil(product.length / page_size)
